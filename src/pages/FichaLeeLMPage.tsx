...
function groupTone(group: QuestionGroup): Tone {
  ...
  if (g.startsWith("ANTES")) return "red";
  if (g.startsWith("DURANTE")) return "amber";
  if (g.startsWith("DESPUES")) return "green";
  return "accent";
}

function sectionBorderClass(group: QuestionGroup) {
  const tone = groupTone(group);
  if (tone === "red") return "border-red-500/30";
  if (tone === "amber") return "border-amber-400/30";
  if (tone === "green") return "border-emerald-400/30";
  return "border-slate-400/30";
}

function sectionTitleClass(group: QuestionGroup) {
  const tone = groupTone(group);
  if (tone === "red") return "text-red-100";
...
export function FichaLeeLMPage() {
  ...
  const draftKey = useMemo(() => {
    ...
    return `${STORAGE_PREFIX}${FICHA_LEE_LM.key}:${uid}`;
  }, [user?.id]);
  ...
  const grouped = useMemo(() => groupBy(FICHA_LEE_LM.preguntas), []);
  ...
  const validate = () => {
    ...
    // preguntas
    for (const q of FICHA_LEE_LM.preguntas) {
      const st = getQ(q.id);
  ...
  async function saveToDatabase() {
    ...
    setSaving(true);
    try {
      // 1) obtener ficha_id: LM 2026 + LEE v1
      ...
      if (!ficha?.id) throw new Error("No existe ficha LEE v1 para LM 2026 (seed).");
      ...
      const answerRows = FICHA_LEE_LM.preguntas.map((q) => {
        const st = getQ(q.id);
        const qkey = toQKeyFromNumero(q.numero);
...
const footerForPdf = {
  ...
  monitor_firma_dni: runPayload.monitor_firma_dni,
      ...
      try {
        ...
        exportFichaEscribeLmPdf({
          titulo: FICHA_LEE_LM.titulo,
          area: FICHA_LEE_LM.area,
          header,
          preguntas: FICHA_LEE_LM.preguntas,
          answers,
          footer: footerForPdf,
          logoDataUrl,
        });
  ...
  const NivelInfo = () => (
    <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-3">
        {FICHA_LEE_LM.encabezado.nivel_avance_info.map((x) => (
          <div
            key={x.nivel}
            className={cls(
  ...
  const [openSections, setOpenSections] = useState<Record<QuestionGroup, boolean>>({
    ANTES_LECTURA: true,
    DURANTE_LECTURA: true,
    DESPUES_LECTURA: true,
    EVALUACION: true,
  });
  ...
  return (
    <div className="text-white">
      ...
      <div className="sticky top-20 z-30 mt-4 flex flex-wrap gap-2 rounded-2xl border border-white/10 bg-zinc-950/80 p-2 backdrop-blur">
        {GROUP_ORDER.map((g) => (
          <button
            key={g}
            type="button"
            onClick={() =>
              document.getElementById(`sec-${g}`)?.scrollIntoView({ behavior: "smooth" })
            }
        ...
        {GROUP_ORDER.map((g) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className={cls("text-lg font-semibold", sectionTitleClass(g))}>
                  {GROUP_LABEL[g]}
                </div>
                <div className="flex items-center gap-2">
              ...
              <div
                className={cls(
                  ...
                  openSections[g] ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
                )}
              >
