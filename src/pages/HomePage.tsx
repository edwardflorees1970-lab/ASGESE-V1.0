        ...
        <div className="mt-3 space-y-2 text-sm text-white/70">
          {runs.slice(0, 6).map((r) => {
            ...
            return (
              <div
                key={r.id}
                className="grid grid-cols-[minmax(0,1fr)_110px_72px] items-center gap-2 rounded-lg border border-white/10 bg-black/20 px-3 py-2"
              >
                <div className="truncate text-white/80" title={`${mon?.nombre || "Monitoreo"} / ${ficha?.codigo || "FICHA"}`}>
                  {mon?.nombre || "Monitoreo"} / {ficha?.codigo || "FICHA"}
                </div>
                <div className="truncate text-right text-xs text-white/50">{fmtDateShort(r.created_at)}</div>
                <div
                  className={cls(
                    "justify-self-end rounded-lg border px-2 py-1 text-xs",
                    r.status === "final"
