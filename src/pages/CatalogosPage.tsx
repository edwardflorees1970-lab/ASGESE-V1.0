...
export function CatalogosPage() {
  ...
  const save = async () => { if (!name.trim()) return; const result = editing ? await supabase.from(table).update({ nombre: name.trim() }).eq("id", editing.id) : await supabase.from(table).insert({ nombre: name.trim() }); if (result.error) return setMessage(result.error.message); setName(""); setEditing(null); setMessage("Catalogo actualizado."); await load(); };
  const remove = async (row: Row) => { if (!window.confirm(`Eliminar ${row.nombre}?`)) return; const { error } = await supabase.from(table).delete().eq("id", row.id); if (error) return setMessage("No se puede eliminar porque esta en uso o no tienes permiso: " + error.message); await load(); };
  return <div className="space-y-5"><header className="agebre-surface rounded-2xl p-5"><div className="text-xs font-bold uppercase tracking-widest text-[var(--app-accent)]">Administracion</div><h1 className="mt-1 text-2xl font-bold text-[var(--app-text)]">Catalogos</h1><p className="mt-1 text-sm text-[var(--app-muted)]">Gestion centralizada de valores usados por instituciones y formularios.</p></header>{message && <div className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-[var(--app-text)]">{message}</div>}<div className="grid gap-5 lg:grid-cols-[18rem_1fr]"><aside className="agebre-surface rounded-2xl p-3">{catalogs.map(([code,label]) => <button key={code} onClick={() => { setTable(code); setEditing(null); setName(""); }} className={`mb-1 w-full rounded-xl px-3 py-2 text-left text-sm ${table===code?"bg-sky-600 text-white":"text-[var(--app-text)] hover:bg-white/5"}`}>{label}</button>)}</aside><section className="agebre-surface rounded-2xl p-4"><div className="flex flex-col gap-2 sm:flex-row"><input value={name} onChange={(e)=>setName(e.target.value)} placeholder="Nombre del valor" className="dashboard-control flex-1 rounded-xl border px-3 py-2"/><button onClick={save} className="rounded-xl bg-sky-600 px-4 py-2 text-sm font-semibold text-white">{editing?"Guardar":"Agregar"}</button>{editing&&<button onClick={()=>{setEditing(null);setName("");}} className="rounded-xl border border-white/10 px-4 py-2 text-sm text-[var(--app-text)]">Cancelar</button>}</div><div className="mt-4 divide-y divide-white/5">{rows.map(row=><div key={row.id} className="flex items-center gap-2 py-3"><span className="flex-1 text-sm text-[var(--app-text)]">{row.nombre}</span><button onClick={()=>{setEditing(row);setName(row.nombre);}} className="rounded-lg border border-white/10 px-3 py-1.5 text-xs text-[var(--app-text)]">Editar</button><button onClick={()=>remove(row)} className="rounded-lg border border-red-400/20 px-3 py-1.5 text-xs text-red-300">Eliminar</button></div>)}</div></section></div></div>;
}
